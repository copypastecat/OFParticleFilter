...
def low_variance_resampler(particles, weights):
    starting_point = default_rng().uniform(0, 1, 1)
    increment = 1 / len(weights)
    ...
    for i in range(len(new_particles)):
        while cumulative_weight < starting_point:
            cumulative_weight += weights[particle_index % len(weights)]
            particle_index += 1

        new_particles[i] = particles[(particle_index - 1) % len(weights)]
